...
- PARTIAL HEM forwards render distance and RC11 certifies that setting transport; RC23 carries the finalized 1.21.5 paletted block/biome no-size-prefix decoder repair after RC21 live evidence exposed chunk-stream desynchronization, and structurally verifies every decoder path present in the frozen historical dependency. A green live rendered-section working set plus exhaustive spawn-chunk/ticket lifecycle and far-view eviction remain to be proven
...
- PARTIAL native 1.21.5 biome generation plus the live renderer path are gated; RC23 deterministically corrects 1.21.5 biome palette no-size-prefix decoding and verifies all discovered decoder paths, but live green evidence and exhaustive biome-color visual comparison remain
