...
Current RC24 evidence note: manifest-verified RC21 reached real Paper 1.21.5 terrain streaming and exposed the block/biome paletted-container decoder failure (`VarInt is too big` / SmartBuffer bounds). RC22 introduced the deterministic no-size-prefix correction; RC23 made decoder-path counting structural but then revealed that the patcher was still scanning unrelated `.pnpm` package-store copies. RC24 patches only `prismarine-chunk` roots resolved by actual runtime consumers while preserving fail-closed decoder coverage on every reachable copy. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
