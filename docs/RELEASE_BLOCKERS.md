...
Current RC39 evidence note: RC38 delivered the strict diagnostic proof required for escalation: Hudson stayed on one physical generation, observed two Paper keepalives and wrote two matching responses, but Paper still timed out Hudson and then Elise. That rules out page-wide counter aggregation and moves the remaining fault below HEM's auth/resume lifecycle. RC39 therefore replaces the active generic `net-browserify` transport with the dedicated `hem-raw-tcp-v1` browser↔gateway↔TCP tunnel. The browser and gateway now expose secret-free ordered frame/byte/close counters, enforce bounded buffering and TCP backpressure, and lock the destination to the configured orchestrator port range. The keepalive gate requires one unchanged physical generation plus browser and gateway byte evidence for at least three Paper keepalive cycles before later gates run. The pinned-live and soak blockers remain OPEN until this exact RC39 build passes the full workflow.
