...
Current RC34 evidence note: the full RC33 CI log proves the visible HTTP 400s are PlayerDB profile lookup misses for HEM synthetic offline usernames, not Paper/proxy errors. RC33 also proves Hudson successfully resumes, Paper sees the `hem:session` channel and logs a newly issued lease, while the refreshed browser reports `resume.received: 0`; the broken boundary is the second server-to-browser custom payload. RC34 removes that dependency by retaining the first five-minute browser-local reconnect lease until its original expiry, while fresh launch authorization revokes any prior reconnect token. RC33 generation-aware presence ordering and exact HTTP diagnostics remain in place. The same RC33 log exposes a separate roughly-one-minute Paper connection timeout that is not claimed fixed by this change. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
