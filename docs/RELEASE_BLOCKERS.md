...
Current RC37 evidence note: RC36 still passed its first keepalive check yet both initial Paper sessions later logged `lost connection: Timed out`, proving the previous server-side `connectionTimeout: 0` change did not clear the underlying transport failure. The exact frozen v0.1.99 lockfile resolves `net-browserify` to commit `e754999ffdea67853bc9b10553b5e9908b40f618`; its browser transport converts each binary WebSocket `Blob` through an independent asynchronous `FileReader` before pushing bytes into the Minecraft TCP stream. RC37 patches that runtime-resolved browser transport to request `ArrayBuffer` frames and process them synchronously in WebSocket message-event order, records the patch in `hem-build.json`, and strengthens live acceptance to require three keepalive round trips per physical browser connection. RC36's idle-timeout policy, RC35's guarded keepalive responder, and RC34's reconnect lease remain in place. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
