...
Current RC31 evidence note: RC30 advanced through two-browser authentication, registry/rendering, client capability provenance, seed authority and settings transport, then both browser sockets began closing while the suite waited for Hudson's first resume lease. RC31 removes the authorization race in the client-requested handshake: `/hem lease` starts only after Paper explicitly confirms `HEM: connected` / `HEM: resumed`, retries are bounded to six attempts at 1.5-second intervals, and both browser + Paper secret-free diagnostics are emitted if delivery still fails. RC30's connection-scoped authorization and RC29's no-trailing-NUL `minecraft:register` patch remain mandatory. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
