...
Current RC30 evidence note: RC29 advanced through the first resume-lease delivery, capability/renderer, seed, settings, and reciprocal remote-skin gates, then timed out only after Hudson refreshed and attempted the resume/rotation lifecycle. RC30 fixes the connection-identity race where an old quit could deauthorize a refreshed Player sharing the same UUID, and changes lease rotation to an explicit client-requested, retryable, idempotent handshake while preserving one-use resume tokens. The RC29 no-trailing-NUL `minecraft:register` patch remains mandatory. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
