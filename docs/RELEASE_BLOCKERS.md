...
Current RC25 evidence note: manifest-verified RC21 reached real Paper 1.21.5 terrain streaming and exposed the block/biome paletted-container decoder failure (`VarInt is too big` / SmartBuffer bounds). RC22 introduced the deterministic no-size-prefix correction; RC23 fixed decoder-path counting; RC24 fixed runtime-root selection. The RC24 GitHub log then identified the actual reachable zardoy `prismarine-chunk` fork (`c5feac83...`) and showed the remaining failure was only RC24's hard-coded `readBuffer(smartBuffer, bitsPerValue)` signature detector. RC25 captures the real second parameter name structurally and applies the same fail-closed 1.21.5 packed-long calculation to every discovered decoder method. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
