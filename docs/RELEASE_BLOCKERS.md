...
Current RC32 evidence note: RC31 passed registry/rendering, client capability provenance, seed authority, settings transport, the initial resume-lease gate and reciprocal remote skins, then failed at the first full-document refresh with Chromium `net::ERR_ABORTED; maybe frame was detached?` while the old game WebSocket was closing. RC32 leaves the now-working lease/auth repairs intact and builds the pinned upstream client with its supported `DISABLE_SERVICE_WORKER=true` switch, with a fail-closed `serviceWorkerDisabled: true` artifact attestation across CI, doctor, live acceptance and deployment. This isolates HEM refresh/reconnect from first-reload PWA takeover. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
