...
Current RC43 evidence note: the exact RC42 Actions run passed the direct Node control, both initial Chromium generations, rendering/capabilities/settings/skins, and the full `session.refresh-resume` gate with gateway-fast-pathed Paper keepalives. The next and only observed failure happened after the acceptance test deliberately stopped the proxy: Paper disconnected the players and Chromium attempted replacement WebSockets, but Hudson's old browser diagnostic generation still reported `endedAt=0`. RC43 makes the raw `hem-raw-tcp-v1` tunnel's terminal `closeReason` authoritative for ending that physical diagnostic generation, so outage lifecycle accounting no longer depends solely on a higher-level Mineflayer/node-minecraft-protocol `end` event. The pinned-live and soak blockers remain OPEN until this exact RC43 build passes the full workflow.
