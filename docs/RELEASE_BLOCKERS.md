...
Current RC33 evidence note: RC32 passed registry/rendering, client capability provenance, seed authority, settings transport, reciprocal remote skins, initial resume-lease delivery, full-document refresh, stored-lease resume attempt, and rotated-lease delivery. The run then failed only at the orchestrator's post-refresh two-player presence counter. RC33 fixes that bookkeeping race by tagging each Paper join with a monotonically increasing physical connection generation and rejecting stale lower-generation disconnect webhooks. It also logs exact HTTP 400 response URLs/bodies during browser acceptance so the remaining anonymous 400 console messages can be attributed instead of guessed at. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
