...
Current RC35 evidence note: the exact RC34 GitHub Actions log proves the browser HTTP 400s are harmless PlayerDB misses for HEM synthetic offline-mode usernames. More importantly, it shows both real Paper sessions being disconnected with `Timed out` before the intended refresh gate, after successful join/render/auth. RC35 addresses that actual longevity boundary with a guarded 1.21.5 keepalive responder: the normal pinned protocol response always wins, and HEM emits one matching fallback only when upstream writes no response. The live certificate now includes a mandatory `client.keepalive-transport` round-trip gate for both browsers. RC34's five-minute reconnect lease and all earlier transport/auth repairs remain in place. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
