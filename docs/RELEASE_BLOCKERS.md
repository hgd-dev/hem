...
Current RC29 evidence note: RC27 advanced the real workflow through ready Paper 1.21.5, rendered chunk sections, the generated sound map/capability contract, seed authority, settings transport, and reciprocal remote custom skins. RC28 then registered `hem:session`, delayed lease delivery until Paper observed the channel, and removed the hard-coded 30-second proxy timeout, but the historical node-minecraft-protocol REGISTER serializer still appended a trailing NUL. RC29 patches that exact runtime dependency after the frozen install so the browser advertises `hem:session` without an empty trailing identifier. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
