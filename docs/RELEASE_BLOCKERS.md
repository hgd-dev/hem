...
Current RC36 evidence note: the exact RC35 workflow passes `client.keepalive-transport` for both browsers, proving the 1.21.5 client can receive and answer Paper keepalives, yet Paper still times both original sessions out later. Root-cause tracing found the HEM websocket/TCP proxy mounted `net-browserify` without timeout options, inheriting its upstream `connectionTimeout = 5000` policy for already-established TCP sockets. RC36 keeps `connectTimeout: 5000` for connection establishment but explicitly sets `connectionTimeout: 0` for established sessions, so Minecraft/Paper keepalives and explicit lifecycle events own long-lived connection health. RC35's guarded keepalive responder and RC34's five-minute reconnect lease remain in place. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow.
