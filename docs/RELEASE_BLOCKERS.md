...
Current RC42 evidence note: the exact RC41 Actions run proved the direct Node control, both initial Chromium generations, rendering/capabilities/settings/skins, and Hudson refresh/resume all work over `hem-raw-tcp-v1`. The refreshed Hudson generation then exposed the remaining failure with unusually strong evidence: the browser reported a **20.079-second maximum event-loop stall** (eight samples over one second), while the raw tunnel had no errors/backpressure and Paper timed the connection out after only one browser-serviced keepalive. RC42 removes the browser main thread from the liveness deadline by fast-pathing the exact Paper 1.21.5 play keepalive at the gateway and dropping the browser's later duplicate. The pinned-live and soak blockers remain OPEN until this exact RC42 build passes the full workflow.
