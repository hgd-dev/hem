...
Current RC38 evidence note: RC37 reached sustained keepalives, settings, skins and refresh/resume, but its page-wide diagnostics could combine observations from a dead physical client and an automatic replacement client. RC38 therefore makes connection state generation-scoped: every physical `bot._client` lifetime gets a monotonically increasing same-tab generation ID, keepalive/error/channel/authentication state is isolated to that generation, and each replacement connection must authenticate once using the retained short-lived resume lease. The keepalive gate now requires one unchanged physical generation for the complete observation window, while refresh and proxy-restart recovery must create fresh resume-authenticated generations. RC37's ordered ArrayBuffer transport patch, RC36's idle-timeout policy, RC35's guarded keepalive responder, and RC34's reconnect lease remain in place. The pinned-live and soak blockers remain OPEN until this exact build passes the full workflow; if an unchanged generation still times out after correct keepalive replies, the next step is a dedicated HEM WebSocket-to-TCP gateway rather than another reconnect heuristic.
