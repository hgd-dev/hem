...
if (cert.upstreamReleaseTag !== 'v0.1.99' || cert.upstreamRelease1215 !== true) throw new Error('Certification is not tied to the known minecraft-web-client v0.1.99 1.21.5 release')
...
if (cert.frozenLockfile !== true) throw new Error('Certification did not use the pinned v0.1.99 frozen lockfile')
if (cert.compatibilityMode !== 'pinned-v0.1.99-lockfile-1215-verified' || cert.protocolVerified1215 !== true) throw new Error(`Certification requires pinned v0.1.99 frozen dependencies plus verified 1.21.5 protocol/data; mode=${cert.compatibilityMode}`)
if (cert.soundMap?.path !== '/sounds.js' || !/^[0-9a-f]{64}$/i.test(cert.soundMap?.sha256 || '') || !Number.isInteger(cert.soundMap?.bytes) || cert.soundMap.bytes < 32) throw new Error('Certification requires generated /sounds.js provenance')
if (cert.prismarineChunkPatch?.patchId !== 'hem-prismarine-chunk-1215-nosize-v5' || cert.prismarineChunkPatch?.reports?.length < 1) throw new Error('Certification requires the HEM 1.21.5 prismarine-chunk no-size-prefix decoder patch')
if (!cert.prismarineChunkPatch.reports.every(report => report.sizing?.blocks5Bits === 342 && report.sizing?.biomes3Bits === 4)) throw new Error('Certification has invalid HEM 1.21.5 chunk-patch sizing evidence')
if (!cert.prismarineChunkPatch.reports.every(report => report.decoderPaths?.readBufferMethods >= 1 && report.decoderPaths?.computedReadPaths === report.decoderPaths?.readBufferMethods)) throw new Error('Certification has invalid HEM 1.21.5 chunk decoder-path coverage evidence')
if (!cert.prismarineChunkPatch.reports.every(report => report.runtimeResolved === true && Array.isArray(report.consumers) && report.consumers.length >= 1)) throw new Error('Certification chunk patch is not tied to runtime-resolved consumers')
