...
test('RC26 patches only runtime-resolved Minecraft 1.21.5 chunk decoders before browser build',()=>{
  const patchPath=path.join(root,'apps/client/patch-prismarine-chunk-1215.mjs')
  assert.equal(fs.existsSync(patchPath),true,'RC26 must ship the deterministic 1.21.5 prismarine-chunk patch')
  const patch=fs.readFileSync(patchPath,'utf8')
  const client=fs.readFileSync(path.join(root,'apps/client/build-client.mjs'),'utf8')
  const system=fs.readFileSync(path.join(root,'tests/system/browser-1215.mjs'),'utf8')
  const cert=fs.readFileSync(path.join(root,'scripts/verify-certification.mjs'),'utf8')
  const deploy=fs.readFileSync(path.join(root,'.github/workflows/deploy-cloudflare.yml'),'utf8')
  assert.match(client,/pnpm\(\['install', '--frozen-lockfile'\]\)[\s\S]*patch-prismarine-chunk-1215\.mjs/,'patch must run only after the frozen v0.1.99 dependency graph is installed')
  assert.match(patch,/hem-prismarine-chunk-1215-nosize-v5/)
  assert.match(patch,/mcData\.version\['>='\]\('1\.21\.5'\)/)
  assert.match(patch,/const methodHeader = \/readBuffer/); assert.match(patch,/const longCount = 'this\.data\.length\(\)'/); assert.match(patch,/packed-long count from allocated BitArray/)
  assert.match(patch,/packedLongCount\(4096, 5\)/)
  assert.match(patch,/packedLongCount\(64, 3\)/)
  assert.match(patch,/readBufferMethods/)
  assert.match(patch,/computedReadPaths/)
  assert.match(patch,/computedReadPaths/)
  assert.match(patch,/findRuntimePrismarineChunkRoots/)
  assert.match(patch,/runtimeResolved: true/)
  assert.match(patch,/consumers: entry\.consumers/)
  assert.match(patch,/topLevelPackageJsons/)
  assert.doesNotMatch(patch,/readdirSync\([^\n]*\.pnpm/,'RC26 must not blindly patch every pnpm store copy')
  assert.match(patch,/failed runtime-resolved prismarine-chunk root/)
  assert.match(client,/prismarineChunkPatch/)
  assert.match(system,/prismarineChunkPatch/)
  assert.match(cert,/prismarineChunkPatch/)
  assert.match(deploy,/prismarineChunkPatch/)
})


test('RC27 ships the upstream-generated sound map and tests remote skins over HTTPS',()=>{
  const client=read('apps/client/build-client.mjs')
  const auth=read('tests/system/auth-stub.mjs')
  const compose=read('tests/system/docker-compose.yml')
  const system=read('tests/system/browser-1215.mjs')
  const cert=read('scripts/verify-certification.mjs')
  const deploy=read('.github/workflows/deploy-cloudflare.yml')
  assert.match(client,/scripts[\\/]downloadSoundsMap\.mjs/)
  assert.match(client,/Pinned v0\.1\.99 sound-map step completed without generating sounds\.js/)
  assert.match(client,/builtSoundMapSha256 !== generatedSoundMapSha256/)
  assert.match(client,/soundMap: \{ source: generatedSoundMapSource, sha256: generatedSoundMapSha256, bytes: generatedSoundMapBytes\.length, path: '\/sounds\.js' \}/)
  assert.match(client,/config\.skinTexturesProxy = ''/)
  assert.match(auth,/https\.createServer/)
  assert.match(auth,/skin-test-key\.pem/)
  assert.match(auth,/skin-test-cert\.pem/)
  assert.match(auth,/HTTPS skin fetch/)
  assert.match(compose,/PUBLIC_SKIN_ORIGIN: https:\/\/127\.0\.0\.1:9443/)
  assert.match(compose,/127\.0\.0\.1:9443:9443/)
  assert.match(system,/ignoreHTTPSErrors: true/)
  assert.match(system,/fetch\('\/sounds\.js'/)
  assert.match(system,/soundMapSha256/)
  assert.match(cert,/generated \/sounds\.js provenance/)
  assert.match(deploy,/generated \/sounds\.js provenance/)
  for(const rel of ['tests/system/tls/skin-test-key.pem','tests/system/tls/skin-test-cert.pem']) assert.equal(fs.existsSync(path.join(root,rel)),true,`${rel} missing`)
})
