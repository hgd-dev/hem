...
test('RC25 patches only runtime-resolved Minecraft 1.21.5 chunk decoders before browser build',()=>{
  const patchPath=path.join(root,'apps/client/patch-prismarine-chunk-1215.mjs')
  assert.equal(fs.existsSync(patchPath),true,'RC25 must ship the deterministic 1.21.5 prismarine-chunk patch')
  ...
  assert.match(patch,/hem-prismarine-chunk-1215-nosize-v4/)
  assert.match(patch,/mcData\.version\['>='\]\('1\.21\.5'\)/)
  assert.match(patch,/const methodHeader = \/readBuffer/); assert.match(patch,/const formula = `Math\.ceil\(this\.data\.capacity \/ Math\.floor\(64 \/ \${method\.bits}\)\)`/)
  ...
  assert.doesNotMatch(patch,/readdirSync\([^\n]*\.pnpm/,'RC25 must not blindly patch every pnpm store copy')
