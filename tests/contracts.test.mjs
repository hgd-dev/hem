...
test('RC24 patches only runtime-resolved Minecraft 1.21.5 chunk decoders before browser build',()=>{
  const patchPath=path.join(root,'apps/client/patch-prismarine-chunk-1215.mjs')
  assert.equal(fs.existsSync(patchPath),true,'RC24 must ship the deterministic 1.21.5 prismarine-chunk patch')
  ...
  assert.match(patch,/hem-prismarine-chunk-1215-nosize-v3/)
  assert.match(patch,/mcData\.version\['>='\]\('1\.21\.5'\)/)
  assert.match(patch,/Math\.ceil\(this\.data\.capacity \/ Math\.floor\(64 \/ bitsPerValue\)\)/)
  ...
  assert.match(patch,/computedReads !== readBufferMethods/)
  ...
  assert.doesNotMatch(patch,/readdirSync\([^\n]*\.pnpm/,'RC24 must not blindly patch every pnpm store copy')
