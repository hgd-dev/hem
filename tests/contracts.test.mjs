...
test('RC23 deterministically repairs Minecraft 1.21.5 chunk and biome palette decoding before browser build',()=>{
  const patchPath=path.join(root,'apps/client/patch-prismarine-chunk-1215.mjs')
  assert.equal(fs.existsSync(patchPath),true,'RC23 must ship the deterministic 1.21.5 prismarine-chunk patch')
  ...
  assert.match(patch,/hem-prismarine-chunk-1215-nosize-v2/)
