...
RC11 targets Minecraft Java 1.21.5 (DataVersion 4325). Before starting an existing Paper world, the orchestrator reads `world/level.dat` and refuses to launch any world whose DataVersion is newer than 4325. **Do not point RC34 at a world that RC9/RC10 already opened with 1.21.11.** Restore a 1.21.5-or-older backup instead. This guard prevents an unsafe Minecraft world downgrade.

## Release status

This source tree is **HEM 1.0.0 RC34**, not a falsely labeled final build.

RC34 continues the **confirmed live Minecraft 1.21.5 runtime repair path**. The complete RC33 CI log proved the anonymous browser HTTP 400s are harmless PlayerDB misses for HEM's synthetic offline-mode usernames, not Paper/proxy failures. It also proved Hudson's refreshed connection successfully resumes and Paper issues a reconnect lease, but the refreshed browser receives zero `hem:session` payloads. RC34 removes that fragile second-delivery dependency: the first private plugin-channel delivery seeds a five-minute, browser-local reconnect lease that may be reused until its original absolute expiry. A fresh `/hem auth` revokes the previous reconnect lease server-side, so the original launch session remains one-use and stale reconnect credentials do not survive a new launch. RC33's generation-aware presence ordering, RC32's service-worker isolation, RC31's authorization-confirmed initial lease request, RC30's connection-scoped authorization, RC29's exact no-trailing-NUL REGISTER patch, the exact v0.1.99 source commit, frozen lockfile, RC26 chunk decoder patch, RC27 sound/HTTPS-skin repairs, RC21 manifest-first checkout protection and repo-root package remain mandatory.
...
Local source verification currently passes **121/121 tests** and **57/57 release contracts**. The final release gate is `.github/workflows/system-1215.yml`, which must successfully:
...
6. prove browser refresh through a five-minute browser-local reconnect lease whose original launch token remains one-use and whose lease is revoked by a fresh launch;
