...
RC11 targets Minecraft Java 1.21.5 (DataVersion 4325). Before starting an existing Paper world, the orchestrator reads `world/level.dat` and refuses to launch any world whose DataVersion is newer than 4325. **Do not point RC31 at a world that RC9/RC10 already opened with 1.21.11.** Restore a 1.21.5-or-older backup instead. This guard prevents an unsafe Minecraft world downgrade.

## Release status

This source tree is **HEM 1.0.0 RC31**, not a falsely labeled final build.

RC31 continues the **confirmed live Minecraft 1.21.5 runtime repair path**. The RC30 live run advanced through two-browser authentication, runtime 1.21.5 registries, rendered chunk sections, client capability provenance, seed authority and settings transport, but both browser sockets began closing while the suite waited for Hudson's first resume lease. RC30 started `/hem lease` immediately beside the asynchronous `/hem auth` command and retried it every 250 ms, allowing up to 40 signed command attempts before authorization/channel readiness was proven. RC31 makes that handshake causal: the browser waits until Paper explicitly reports `HEM: connected` or `HEM: resumed`, marks authorization complete, then issues at most six secret-free lease requests at 1.5-second intervals. Paper-side logs now record only secret-free channel milestones (`registered`, `waiting`, `issued`) and the initial acceptance gate dumps those diagnostics plus a Paper tail on failure. RC30's connection-scoped authorization and RC29's exact no-trailing-NUL REGISTER patch remain mandatory. The exact v0.1.99 source commit, frozen lockfile, RC26 chunk decoder patch, RC27 sound/HTTPS-skin repairs, RC21 manifest-first checkout protection and repo-root package remain unchanged.
...
Local source verification currently passes **110/110 tests** and **57/57 release contracts**. The final release gate is `.github/workflows/system-1215.yml`, which must successfully:
