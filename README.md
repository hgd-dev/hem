...
RC11 targets Minecraft Java 1.21.5 (DataVersion 4325). Before starting an existing Paper world, the orchestrator reads `world/level.dat` and refuses to launch any world whose DataVersion is newer than 4325. **Do not point RC32 at a world that RC9/RC10 already opened with 1.21.11.** Restore a 1.21.5-or-older backup instead. This guard prevents an unsafe Minecraft world downgrade.

## Release status

This source tree is **HEM 1.0.0 RC32**, not a falsely labeled final build.

RC32 continues the **confirmed live Minecraft 1.21.5 runtime repair path**. The RC31 live run passed two-browser authentication, runtime 1.21.5 registries, rendered chunk sections, client capability provenance, seed authority, settings transport, the initial short-lived resume-lease gate, and reciprocal remote-skin rendering. It then failed exactly when System Acceptance performed the first full-document refresh: Chromium reported the client document request as `net::ERR_ABORTED; maybe frame was detached?` while the old game WebSocket was closing. RC32 leaves the now-working auth/lease path unchanged and uses the pinned upstream project's supported `DISABLE_SERVICE_WORKER=true` production switch so a newly installed PWA worker cannot take over or supersede HEM's first refresh navigation. `hem-build.json`, CI, doctor, live acceptance, and production deployment now all require an explicit `serviceWorkerDisabled: true` attestation. RC31's authorization-confirmed lease request, RC30's connection-scoped authorization, RC29's exact no-trailing-NUL REGISTER patch, the exact v0.1.99 source commit, frozen lockfile, RC26 chunk decoder patch, RC27 sound/HTTPS-skin repairs, RC21 manifest-first checkout protection and repo-root package remain mandatory.
...
Local source verification currently passes **111/111 tests** and **57/57 release contracts**. The final release gate is `.github/workflows/system-1215.yml`, which must successfully:
