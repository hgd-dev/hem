...
RC11 targets Minecraft Java 1.21.5 (DataVersion 4325). Before starting an existing Paper world, the orchestrator reads `world/level.dat` and refuses to launch any world whose DataVersion is newer than 4325. **Do not point RC33 at a world that RC9/RC10 already opened with 1.21.11.** Restore a 1.21.5-or-older backup instead. This guard prevents an unsafe Minecraft world downgrade.

## Release status

This source tree is **HEM 1.0.0 RC33**, not a falsely labeled final build.

RC33 continues the **confirmed live Minecraft 1.21.5 runtime repair path**. The RC32 live run passed two-browser authentication, runtime 1.21.5 registries, rendered chunk sections, client capability provenance, seed authority, settings transport, reciprocal remote-skin rendering, the initial resume lease, the browser refresh itself, the stored-lease resume attempt, and rotated-lease delivery. It then failed only at the orchestrator's post-refresh `world has 2 authenticated players` counter. That counter was keyed by username and ordered only by webhook timestamps, so a delayed quit from Hudson's pre-refresh physical connection could overwrite the newer refreshed connection. RC33 assigns a monotonically increasing connection generation to every Paper join and makes orchestrator presence generation-aware: stale lower-generation disconnects are ignored even if their HTTP callbacks arrive later. The live acceptance runner also prints the exact URL and short response body for every HTTP 400 so anonymous browser-console 400s are no longer treated as root-cause evidence. RC32's service-worker isolation, RC31's authorization-confirmed lease request, RC30's connection-scoped authorization, RC29's exact no-trailing-NUL REGISTER patch, the exact v0.1.99 source commit, frozen lockfile, RC26 chunk decoder patch, RC27 sound/HTTPS-skin repairs, RC21 manifest-first checkout protection and repo-root package remain mandatory.
...
Local source verification currently passes **118/118 tests** and **57/57 release contracts**. The final release gate is `.github/workflows/system-1215.yml`, which must successfully:
