...
RC11 targets Minecraft Java 1.21.5 (DataVersion 4325). Before starting an existing Paper world, the orchestrator reads `world/level.dat` and refuses to launch any world whose DataVersion is newer than 4325. **Do not point RC27 at a world that RC9/RC10 already opened with 1.21.11.** Restore a 1.21.5-or-older backup instead. This guard prevents an unsafe Minecraft world downgrade.

## Release status

This source tree is **HEM 1.0.0 RC27**, not a falsely labeled final build.

RC27 continues the **confirmed live Minecraft 1.21.5 runtime repair path**. RC26 successfully moved System Acceptance past the historical chunk-decoder crash: Paper reached `ready`, both Chromium clients joined far enough to transport HEM settings, and the next concrete blockers were a missing upstream `/sounds.js` asset plus custom-skin fetches hitting an HTTP-only local test origin and failing with `ERR_SSL_PROTOCOL_ERROR`. RC27 now runs the pinned upstream `scripts/downloadSoundsMap.mjs` production step, requires the generated `sounds.js` to survive into `dist/`, hashes it into `hem-build.json`, and live-checks the served asset. The system skin stub now exposes the distinct Hudson/Elise PNGs from a dedicated local HTTPS origin with a test-only certificate and Playwright explicitly accepts that certificate; production `skinTexturesProxy` remains disabled so profile texture URLs are fetched directly. The exact v0.1.99 source commit, frozen lockfile, RC26 chunk decoder patch, runtime-root attestation, RC21 manifest-first checkout protection and repo-root package remain unchanged.
...
Local source verification currently passes **99/99 tests** and **56/56 release contracts**. The final release gate is `.github/workflows/system-1215.yml`, which must successfully:
