...
RC28 continues the **confirmed live Minecraft 1.21.5 runtime repair path**. RC27 proved the chunk decoder, generated sound map, live rendered chunk sections, HEM settings transport, and reciprocal remote custom skins against real Paper 1.21.5. The next blocker was HEM's own refresh-session lifecycle: both WebSocket connections ended before the browser stored the short-lived resume lease. RC28 explicitly registers the `hem:session` raw plugin channel from the browser before authorization, waits on Paper until that channel appears in `Player#getListeningPluginChannels()` before issuing a lease, and listens through both the named channel event and generic payload fallbacks. The proxy no longer imposes HEM's former hard-coded 30-second bridge timeout, so Minecraft keepalives and HEM lifecycle controls govern long-lived sessions needed by the 60-minute soak. The exact v0.1.99 source commit, frozen lockfile, RC26 chunk decoder patch, RC27 sound/HTTPS-skin repairs, RC21 manifest-first checkout protection and repo-root package remain unchanged.
